...
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val db = Room.databaseBuilder(
            ...
            .fallbackToDestructiveMigration() // Évite le crash en cas de changement de schéma
            .build()
...
fun FoodAnalyticApp(userDao: UserDao, productDao: ProductDao) {
    ...
    NavigationSuiteScaffold(
        ...
        when (currentDestination) {
            AppDestinations.HOME -> HomeScreen(productDao)
            AppDestinations.FAVORITES -> FavoritesScreen(productDao)
